...
<v-toolbar color="primary">
    <v-card-title>
        <h1 style="color:white">Name your Beneficiaries Successors</h1>
        <v-spacer></v-spacer>
    </v-card-title>
</v-toolbar>
<v-container fluid class="px-3">
    <v-layout row wrap>
            ...
            <v-form v-model="valid" ref="form" lazy-validation>
                <ol class="flex md xs12  ">
                    <li   v-for="response in question.responses" >
                        <h3 v-if="response.inputType == 'secondChoice'" >{{response.option}}</h3>
                        ...
                        <div v-bind:id="response.id" v-if="response.inputType == 'datePicker'">
                            <v-menu ref="menu" @click.native="getDate" lazy offset-y full-width :close-on-content-click="false"
                                ...
                                <v-date-picker ref="picker" v-bind:min="mindate"  v-bind:max="maxdate" v-model="question.dateofbirth">
                        ...
                        <div v-bind:id="response.id" v-if="response.inputType == 'datePicker2'">
                            <v-menu ref="menutwo" @click.native="getDate" lazy offset-y full-width :close-on-content-click="false"
                                    v-model="menutwo" transition="scale-transition" v-bind:next="response.next"
                                    v-bind:nudge-right="40"  min-width="290px">
                                <v-text-field slot="activator" v-bind:label="response.option"
                                              v-bind:value="question.dateofbirth2"
                                              @focus="getUserInput" v-bind:rules="arrvalidation.birthRules"
                                              v-bind:next="response.next"
                                              required v-model="question.dateofbirthtwo" prepend-icon="event"
                                              readonly ></v-text-field>
                                <v-date-picker ref="pickertwo" v-bind:min="mindate"  v-bind:max="maxdate" v-model="question.dateofbirthtwo">
                                    <template slot-scope="{ save, cancel }">
                                        <v-card-actions>
                                            <v-spacer></v-spacer>
                                            <v-btn flat color="primary"  @click="menu = false">Cancel</v-btn>
                                            <v-btn flat color="primary"  @click="$refs.menutwo[0].save(date)">OK</v-btn>
                                        </v-card-actions>
                                    </template>
                                </v-date-picker>
                            </v-menu>
        ...
        <child v-if="!store.state.IsloginComponents">
        </child>
        <login v-if="Islogin" >
        </login>
    </v-layout>
...
<script>
    ...
    export default {
        name: "BeneficiariesSuccessors",
        components: {
            login,child
        },
        data: function () {
            return {maxdate:"2000-01-01",mindate:"1950-01-01",datetwo:null,
                Islogin:false,
                Questions:[],subchild:[],//showsubchild:false,
                progressshow: true,isvaluefilled:false,
                date: null,menutwo:false,
                menu: false,valid:false,selectinput:false,
                Name:'',
                store: store, currentQuestion:0,Beneficiaries:0, //userResponses:'',
            ...
            menutwo (val) {
                val && this.$nextTick(() => (this.$refs.pickertwo[0].activePicker = 'YEAR'))
            }
        },beforeMount(){
        },
        created: function() { // this.$store.state.currentQuestion=9;
           ...
           console.log('1');
             ...
             if (localStorage.getItem('Beneficiaries') !== 'undefined' && localStorage.getItem('Beneficiaries') !== null) {
                this.progress = store.state.currentQuestion
                this.currentQuestion=JSON.parse(localStorage.getItem('Beneficiaries'));
                ...
                console.log(this.$store.state.survey.BeneficiariesSuccessors);
             }
        },
        computed: {
            ...mapState(['survey']),
            ...mapGetters([
                ...
                get() {
                    return this.$store.state.survey.BeneficiariesSuccessors[state.currentQuestion].userResponses;
                }, set(value) {
                    this.$store.commit("updateBeneficiaries", value);
                }
            }
        }, mounted: function () {
            ...
            updateResponses (e){
                ...
                this.$store.commit('updateBeneficiaries', this.event.target.value)
                if (this.$store.state.survey.BeneficiariesSuccessors[this.currentQuestion].nextChild=='secondChild') {
                    if (this.$store.state.survey.BeneficiariesSuccessors[this.currentQuestion].userResponses === 'Yes') {
                        //this.$store.commit('isValid', this.valid)
                        //this.$store.commit('updateBeneficiaries', this.event.target.value)
                    }
                }else{
                    ...
                    localStorage.setItem('Beneficiaries',this.currentQuestion);
                } catch (err) {
            ...
            goToNextQuestion() {  console.log(this.$refs.form[0].validate());
                if (this.$refs.form[0].validate()) {
                    ...
                    try {
                        const serializedState = JSON.stringify(this.store.state);
                        localStorage.setItem('quiz', serializedState);
                        localStorage.setItem('Beneficiaries', JSON.stringify(this.currentQuestion));
